...
class HeartAnimationViewModel: ObservableObject {
    ...
    private func scheduleNextBeat() {
        ...
        // 高精度タイマーでスケジュール
        beatTimer = Timer.scheduledTimer(withTimeInterval: actualInterval, repeats: false) { [weak self] _ in
    ...
    private func generateHRVVariation() -> Double {
        let randomVariation = Double.random(in: -hrvVariabilityPercent...hrvVariabilityPercent) / 100.0
